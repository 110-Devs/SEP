...
const attrJSON = function (node: any, opts: any) {
  ...
  attributes['data-selector'] = getCssSelector(node);
...
const getCssSelector = (el: Element): string => {
  let path: string[] = [], parent: Element | null;
  
  // Helper function to check if an ID is valid for use in CSS selectors
  const isValidId = (id: string) => /^[_a-zA-Z][_a-zA-Z0-9-]*$/.test(id);

  while (parent = el.parentNode as Element | null) {
    let tag: string = el.tagName.toLowerCase(), siblings: HTMLCollection;
